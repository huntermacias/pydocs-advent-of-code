...
import "ace-builds/src-noconflict/theme-monokai"; // Import AceEditor themes
	  ...
	  <div className="lg:w-1/2 p-8 border-r lg:border-gray-700">
		...
		<div className="mb-8">
		  ...
		  {challenge.topics.map(topic => (
			<Badge key={topic} className="mr-2 mb-2 bg-blue-700 hover:bg-blue-600">
			  {topic}
			</Badge>
		  ))}
		</div>
      ...
      <div key={index} className="bg-gray-800 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          <div>
            ...
            <pre className="bg-gray-700 text-white p-2 rounded">{testCase.input}</pre>
          </div>
          <div>
            <p className="font-medium text-sm">Expected Output:</p>
            <pre className="bg-gray-700 text-white p-2 rounded">{testCase.output}</pre>
          </div>
        </div>
        <button 
